  ...
  <div>
    <div class="fv-row mb-7">
      <label class="form-check form-switch form-check-custom form-check-solid">
        <input
          ...
          :disabled="store.forms.create_update_test_module == 'see'"
        />
        <span
    ...
    <div class="fv-row mb-7">
      ...
      <input
        ...
        placeholder="Nombre"
        :readonly="store.forms.create_update_test_module == 'see'"
      />
    ...
    <div align="right">
      ...
      <button
        v-if="store.forms.create_update_test_module != 'see'"
        @click="handleSubmit()"
        type="button"
...
const handleSubmit = async () => {
  ...
  store.forms.create_update_test_module == "create"
    ? store.createTestModule(form_state)
    : store.updateTestModule(form_state, store.items.test_module.selected_item.id);
};

const setForm = () => {
  //API WARNING
  //SELECTED ITEM COMES WITH API ATTRIBUTE NAMES
  form_state.name = store.items.test_module.selected_item.name;
  form_state.is_active = store.items.test_module.selected_item.is_active;
};

onMounted(() => {
  if (store.forms.create_update_test_module != "create") setForm();
});
</script>

<script>
export default {
  name: "TestModuleFormComponent",
};
</script>
