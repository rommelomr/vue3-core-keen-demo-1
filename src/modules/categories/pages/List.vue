      ...
      <div class="row g-5 g-xl-10 mb-5 mb-xl-10">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div v-if="store.items.categories.list.length > 0">
              <div class="table-responsive">
                ...
                >
                  ...
                  <tbody class="fw-semibold text-gray-600">
                    <tr
                      v-for="(item, i) in store.items.categories.list"
                      :key="'item_' + i"
                    >
                      <!--begin::Checkbox-->
                      <td>
                          ...
                          >
                            ...
                            <li class="menu-item">
                              <a
                                href="#"
                                class="menu-link px-3"
                                @click="deleteCategories(item, item.id)"
                                >Eliminar</a
                              >
                            </li>
                          </ul>
              ...
              <div class="mt-6">
                ...
                <div style="float: right">
                  <Paginator
                    v-if="
                      Math.ceil(store.items.categories.paginator.total_pages) >
                      1
                    "
                    :key="store.items.categories.paginator.key"
                    @onChangePage="onChangePage"
                    :items-per-page="
                      store.items.categories.paginator.items_per_page
                    "
                    :max-buttons="5"
                    :total-items="store.items.categories.paginator.total_items"
                    :total-pages="
                      Math.ceil(store.items.categories.paginator.total_pages)
                    "
                    :current-page="
                      store.items.categories.paginator.current_page
                    "
                  />
                </div>
              </div>
        ...
        >
          <div class="modal-dialog modal-dialog-scrollable modal-lg">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <CategoriesForm
                  v-if="store.forms.create_update_categories != ''"
                />
              </div>
          ...
          <!-- <CategoriesForm v-show="store.modal != ''" /> -->
        </div>
...
const deleteCategories = (item, id) => {
  store.setSelectedItem("categories", item);
  confirm(swal, "¿Está seguro que desea eliminar este elemento?", () => {
    store.deleteCategories(id);
  });
};
...
const checkUncheckAll = (v) => {
  if (v.target.checked) {
    ...
    store.items.categories.list.map((item) => {
...
let items_per_page = ref(store.items.categories.paginator.items_per_page);
...
const createCategories = () => {
  store.setModal("create_categories");
  store.setModalTitle("Agregar categories");
  store.setFormMode("create_update_categories", "create");
};
const seeCategories = (item, index) => {
  store.setModal("see_categories");
  store.setModalTitle("Ver datos de categories");
  store.setFormMode("create_update_categories", "see");
  store.setSelectedItem("categories", item);
  store.setSelectedItemIndex("categories", index);
};

const updateCategories = (item, index) => {
  store.setModal("update_categories");
  store.setModalTitle("Editar categories");
  store.setFormMode("create_update_categories", "update");
  store.setSelectedItem("categories", item);
  store.setSelectedItemIndex("categories", index);
};
...
const closeCategoriesModal = () => {
  store.setModal("");
  store.setModalTitle("");
  store.setFormMode("create_update_categories", "");
  store.setSelectedItem("categories", {});
  store.setSelectedItemIndex("categories", -1);
};
</script>
